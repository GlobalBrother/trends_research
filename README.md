...
### Post-verification gaps (found 2026-04-22, must close before the demo)
...
- **Problem.** Roadmap 2.5 moved endpoint handlers from `src/api/main.py` into `src/api/routes/*.py`. `tests/test_api.py` still calls `unittest.mock.patch("src.api.main.<symbol>")` for attributes that now live in the router modules. The three failing tests are `test_get_clusters`, `test_cluster_detail_and_feedback`, `test_reports_and_backtests`, all raising `AttributeError` on patch setup.
- **Fix.** Re-point each `patch(...)` to the symbol's new home (likely `src.api.routes.content` or `src.api.routes.niches`, depending on the symbol). Do **not** re-export the symbol from `main.py` to paper over the test — that re-entangles what 2.5 just separated.
- **Impact.** Demo-blocking confidence: the only three tests that actually exercise cluster + reports endpoints are broken, so we have no post-refactor coverage on the exact surface the demo shows.
- **Effort.** 15–30 minutes.
- **Prompt for Opus 4.7.**
  > Fix the 3 new errors in `tests/test_api.py` that were introduced by Roadmap 2.5 (the FastAPI router split). The baseline per `.github/copilot-instructions.md` is 2 failed + 23 errors; the current tree is 5 failed + 26 errors. All 3 new errors (`test_get_clusters`, `test_cluster_detail_and_feedback`, `test_reports_and_backtests`) are `AttributeError` at patch time.
  > 1. Read `tests/test_api.py` and list every `patch("src.api.main.<X>")` and `patch.object(src.api.main, "<X>", ...)` call.
  > 2. For each symbol, grep `src/api/routes/` to find where it lives **now** and update the patch target to the new module (e.g. `patch("src.api.routes.content.get_session")`). If a symbol is called as `main.X` inside the test body, either import it from its new module or update the call site.
  > 3. Do NOT re-export the moved symbols from `src.api.main` to make the old patch targets work again — that defeats the split.
